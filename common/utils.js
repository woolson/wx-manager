...
// obj: [1, 2, 3], has(obj, 1) => true
// obj: {abc: 123}, has(obj, '123') => true
export function has (obj, keys) {
	const keyType = getType(keys)

	switch (getType(obj)) {
		case 'object':
			if (keyType === 'string') return obj.hasOwnProperty(keys)
			else return keys.some(o => obj.hasOwnProperty(o))
		case 'array':
		case 'string':
			if (keyType === 'array') {
				return keys.some(o => obj.indexOf(o) !== -1)
			} else return obj.indexOf(keys) !== -1
		default:
			return false
	}
}
